...
BOOM::Chi2Table::Chi2Table(BOOM::String filename)
  : DFs(35), alphas(12), table(12,35)
{
  if(filename=="")
  ...
  else
    {
      ...
      if(!file.good()) throw BOOM::String("Error opening file: ")+filename;
      loadFromStream(file);
    }
}
...
void BOOM::Chi2Table::loadFromStream(istream &file)
{
  int row=0;
  ...
  while(!file.eof())
    {
      line.getline(file);
      if(file.eof()) break;
      BOOM::Vector<BOOM::String> *fieldsPtr=line.getFields();
      if(!fieldsPtr) continue;
      BOOM::Vector<BOOM::String> &fields=*fieldsPtr;
      int n=fields.size();
...
float BOOM::Chi2Table::lookupChi(int df,float alpha)
{
...
float BOOM::Chi2Table::lookupP(int df,float chi)
{
  int dfIndex=dfToIndex(df);
...
unsigned BOOM::Chi2Table::alphaToIndex(float alpha)
{
  unsigned begin=0, end=alphas.size();
  while(begin<end)
    {
  ...
  if(alphas[begin]!=alpha) 
    throw BOOM::String("Bad alpha in BOOM::Chi2Table::lookupChi(")+
      alpha+")";
  return begin;
}
...
unsigned BOOM::Chi2Table::dfToIndex(int df)
{
